...
/**
 * Worker class which compares the data coming from the website and the data cached in the local
 * files to detect whether an update is due.
 *
...
public class NotifWorker extends Worker {
    ...
    private boolean checkFeed() {
        ...
        if (news.size() == 1)
            sendNotificationFeed("Duca Degli Abruzzi | Nuova notizia", news.get(0));
    ...
    private boolean checkCircolari() {
        ...
        CircolariParser remoteParser = new CircolariParser();
        CircolariParser localParser = new CircolariParser();
        List<Filterable> remote_items, local_items;
        try {
            remote_items = remoteParser.parse(remote_is);
            local_items = localParser.parse(local_is);
        } catch (XmlPullParserException e) {
            return false;
        } catch (IOException e) {
            return false;
        ...
        for (Filterable f : remote_items) {
            if (!local_items.contains(f)) news.add(f);
        }
        if (news.size() == 1)
            sendNotificationCircolari("Duca degli Abruzzi | Nuova Circolare",
                    ((Circolare) news.get(0)).titolo);
    ...
    public void sendNotificationFeed(String title, FeedItem item) {
        ...
        NotificationCompat.Builder notification =
                new NotificationCompat.Builder(getApplicationContext(), "DucaApp7455")
    ...
    public void sendNotificationCircolari(String title, String description) {
        ...
        NotificationManager notificationManager =
                (NotificationManager) getApplicationContext().getSystemService(Context.NOTIFICATION_SERVICE);
        NotificationCompat.Builder notification =
                new NotificationCompat.Builder(getApplicationContext(), "DucaApp2690")
                        .setContentTitle(title)
    ...
    public void sendBigNotificationCircolari(List news) {
        ...
        NotificationCompat.Builder notification =
                new NotificationCompat.Builder(getApplicationContext(), "DucaApp2690")
                        ...
                        .setAutoCancel(true);
        if (news.size() <= 6)
            notification.setContentText("Ci sono " + news.size() + " nuove circolari");
    ...
    public void sendBigNotificationFeed(List news) {
        ...
        NotificationCompat.Builder notification =
                new NotificationCompat.Builder(getApplicationContext(), "DucaApp7455")
                        ...
                        .setAutoCancel(true);
        if (news.size() <= 6)
            notification.setContentText("Ci sono " + news.size() + " nuove notizie");
